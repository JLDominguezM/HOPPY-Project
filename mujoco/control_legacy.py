...
# ---- parametros afinados (ciclo limite con salto REAL verificado) ----
# Hallados con metrica honesta: un salto cuenta solo si el pie despega >4 cm
# y el cuerpo se mantiene erguido (sin colapso del gantry). El resorte de
# rodilla rigido (rebote tipo pogo) + poste alto fueron la clave.
PARAMS = dict(DEFAULTS)
PARAMS.update(dict(knee_stiff=30.0, spring_ref=-0.8, fz_scale=1.0, cw_mass=1.9,
                   post_h=0.24, tst=0.15, kp_z=1400.0))
...
def run(params=PARAMS, t_total=8.0):
    ...
    apexes = []; cur_apex = -9.9; clears = []; cur_clear = 0.0; t = 0.0; bz_prev = None
    for step in range(int(t_total / DT)):
        ...
        for k, v in zip(["t", "phase", "body_z", "foot_z", "tau3", "tau4", "V3", "V4",
            ...
            L[k].append(v)
        # FSM robusta; se registra el despegue del pie (clearance) por ciclo
        if phase == "stance":
            if (t - t_td) >= p['tst'] or fz_site > 0.02:
                phase = "aerial"; t_lo = t; cur_apex = bz; cur_clear = fz_site
        else:
            cur_apex = max(cur_apex, bz); cur_clear = max(cur_clear, fz_site)
            if contact and (t - t_lo) > 0.03:
                phase = "stance"; t_td = t
                apexes.append(cur_apex); clears.append(cur_clear)
        mujoco.mj_step(m, d); t += DT
        if np.any(np.isnan(d.qpos)): break
    return {k: np.array(v) for k, v in L.items()}, np.array(apexes), np.array(clears)


def figuras(L, apexes, clears):
    import os; os.makedirs("figuras", exist_ok=True)
    t = L["t"]
    real = int(np.sum(clears > 0.04))   # salto REAL: el pie despega >4 cm
    fig, ax = plt.subplots(3, 2, figsize=(12, 9))
    # 1) altura del cuerpo (salto sostenido)
    ax[0, 0].plot(t, L["body_z"], "b"); ax[0, 0].grid(True)
    ax[0, 0].set_title(f"Altura del cuerpo ({real} saltos reales, pie despega max={clears.max()*1000:.0f} mm)")
...
if __name__ == "__main__":
    L, apexes, clears = run()
    real = int(np.sum(clears > 0.04))
    ss = slice(len(L['body_z']) // 3, None)   # regimen (descartar transitorio)
    print(f"Saltos REALES (pie despega >4cm): {real} de {len(clears)} ciclos")
    print(f"Despegue del pie: max={clears.max()*1000:.0f}mm  medio={clears.mean()*1000:.0f}mm")
    print(f"Altura cuerpo en regimen: [{L['body_z'][ss].min():.3f}, {L['body_z'][ss].max():.3f}] m (nominal {PARAMS['post_h']:.2f})")
