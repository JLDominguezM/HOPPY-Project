"""Renderiza un video del salto de HOPPY (M6). Backend EGL (headless)."""
import os
...
def main():
    ...
    cam.lookat[:] = [0.3, 0.0, 0.15]; cam.distance = 1.6
    cam.azimuth = 90; cam.elevation = -12

    qd_filt = np.zeros(2); q_prev = None
    phase = "stance"; t_td = 0.0; t_lo = -1.0
    frames = []; t = 0.0; next_frame = 0.0
    for step in range(int(T_TOTAL / DT)):
        ...
        if t >= next_frame:
            ...
            frames.append(ren.render()); next_frame += 1.0 / FPS
