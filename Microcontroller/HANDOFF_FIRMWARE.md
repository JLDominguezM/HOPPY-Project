...
## MORFOLOGÍA REAL DE LA PIERNA (aclarada 2026-06-10 - INVALIDA supuestos anteriores)
...
## ETAPA 3 - JUMP_MODE escrito (2026-06-11), pendiente banco (y contrapeso para despegar)

FSM de salto continuo agregada a `cpu01_main.c` (lógica verificada con simulación sintética:
touchdown por flanco + debounce, liftoff temprano al soltar el sensor, telemetría correcta):
- **Ciclo**: AÉREO (PD a pose de aterrizaje `traj_x0/traj_depth` - colocación FIJA; sin encoder
  de boom no hay Raibert, el avance lo fija `Fx` como la sim con `vx_d=0`) → **touchdown**
  (flanco 0→1 del sensor con `t_air ≥ lo_debounce=0.04`) → **APOYO** (empuje Bézier de Etapa 2)
  → **liftoff** (sensor suelto tras `td_min_stance=0.05` = el "GRF<umbral" del paper, o fin de
  `Tst`) → AÉREO →...
- **Knobs nuevos**: `JUMP_MODE` (1=saltar; default 0), `lo_debounce`, `td_min_stance`.
- **Telemetría**: `n_hops` (contador), `t_air_last` (duración del último VUELO → altura apex
  ≈ g·t²/8: 0.2 s = 5 cm, 0.3 s = 11 cm), `t_st_last` (duración real del último apoyo, para
  afinar `Tst`). Todo legible en Expressions después de cada salto.
- `MOTORS_OFF` y `u_lim_test` siguen mandando. `STANCE_TEST`/`st_go`/`st_auto`/`st_sensor`
  (Etapa 2) intactos e independientes.

**Protocolo de banco SIN contrapeso ("botecitos" - valida toda la lógica aunque no despegue):**
1. Afinar `analog_limit` primero (presionar el pie al piso en el ángulo real de aterrizaje,
   ver `analog_in[0]`, elegir umbral que separe limpio; hoy 2048).
2. `traj_depth=−0.18`, `fx_scale=0` (en sitio), `Tst=0.35`, `u_lim_test=8-10`, `MOTORS_OFF=0`.
3. Bajar el boom hasta que el pie alcance el piso en la cuclilla → `JUMP_MODE=1` → debe
   ciclar solo: empuje al contacto → recoge en vuelo → cae → empuje... Ver `n_hops` subir y
   `t_air_last`/`t_st_last` por salto.
4. Si dispara en falso o no dispara: ajustar `analog_limit` / `lo_debounce` en vivo.
5. Con contrapeso (cuando llegue): `fx_scale=1` para avanzar; el signo de `fx_scale` fija el
   sentido. Esperado del vuelo: `t_air_last` ≈ 0.2-0.3 s si salta como la sim.

**Pendiente para el salto real**: contrapeso (medir el exacto con el método del boom horizontal:
peso a 35 cm que lo deja flotando = 100%; conseguir el 76% de eso), y verificación del sensor
de pie bajo impacto real (pendiente mecánico diferido).

### Bitácora banco 2026-06-11 (tarde) -  PRIMEROS SALTOS AUTÓNOMOS + lección de impactos

**EL HOPPY FÍSICO SALTÓ SOLO**: JUMP_MODE funcionando end-to-end - touchdown por sensor,
empuje GRF, vuelo, recuperación, re-touchdown. Telemetría de la mejor corrida: **n_hops=33
acumulados**, `t_air_last` hasta **0.18 s** (~4 cm de apex), `pk_pwm=(10,10)` = motores
saturados a 12 V como la sim, rodilla absorbiendo aterrizajes hasta e=−1.01 y empujando con
1.5 rad de carrera. Video del rebote sostenido (~14 ciclos a ~1.3 Hz) analizado cuadro a cuadro.

**Setup físico que funcionó**: contrapeso 5 kg @ 19 cm (a 23 cm sobre-balanceaba: el cuerpo
se levantaba solo - el momento real del lado hopper es MENOR que el estimado de 1.76 kg·m).
Robot sobre mesa, pie sobre tabla. Arranque: levantar el cuerpo (pie descargado) → JUMP_MODE=1
→ presionar firme una vez → soltar.

**Calibración del sensor de pie (resuelta, valores reales)**: aire=0, rozando=~200, reposando
cargado=~2870, presionado=4095. `analog_limit=2048` es correcto (NO bajarlo a 1000: los roces
en vuelo leen 600-1100 y disparan touchdowns falsos). Que el sensor toque antes que el regatón
ES el diseño: roce ligero=sensa, carga=el regatón engancha.

**Trims aprendidos**: `traj_depth=−0.18` es CRÍTICO (con −0.22 aterriza casi estirada, 1/3 de
carrera → brincos de 4 cm "que intentan"; ojo: los reinicios de CCS regresan los knobs a
default −0.22). `traj_x0` = trim de la deriva (sin encoder de boom no hay Raibert; −0.02 usado).
Deriva hacia adelante en sitio = esperada, sin control de posición.

**Fallas encontradas (las 2 de vibración/impacto):**
1. **Runaway del motor de cadera** al cambiar un knob: el conector del encoder de cadera quedó
   mudo (falso contacto aflojado por los brincos) → PD con error constante → giro continuo.
   Se re-asentó y aseguró. Lección: los conectores de encoder se ASEGURAN (cinta/silicón/cincho)
   antes de cada sesión de salto.
2. **LOS ACOPLES PATINAN CON LOS IMPACTOS DE ATERRIZAJE** (donde se pausó): tras 33 aterrizajes,
   en la pose cero física q_now=(−1.23, −0.72) - AMBOS ceros corridos; raya de marcador del
   acople de rodilla DESALINEADA confirmada (cadera por confirmar). Con el cero corrido la IK
   apunta chueco → liftoffs espurios (t_st_last=0.164) y brinquitos (t_air=0.07). REPARACIÓN:
   prisionero al plano D + Loctite CURADO (horas) mínimo; ideal segundo prisionero a 90° o
   pasador pasante (a prueba de impactos). pk_e_min=−3.08 (imposible físico) fue el delator.

**Ritual nuevo obligatorio**: verificar el cero (pose cero a mano → q_now=(0,0)±0.03) ANTES y
DESPUÉS de cada corrida de saltos - cacha el deslizamiento en 1 corrida en vez de 33.

**Siguiente sesión**: reparar acoples (Loctite curado) → re-zero → corrida con cero estable →
medir t_air_last típico con cuclilla profunda → luego `fx_scale=1` (avance alrededor del poste;
si va al revés, fx_scale=−1) → afinar `traj_x0`/`Tst` con la telemetría.
...
3. **Etapa 3 (salto en piso):** la FSM ya está ESCRITA (`JUMP_MODE`, sección arriba) y la
   lógica verificada en sim sintética. Falta en banco: (a) afinar `analog_limit` del sensor,
   (b) probar JUMP_MODE "a botecitos" sin contrapeso, (c) medir y conseguir el contrapeso
   (boom horizontal: 100% → comprar el 76%), (d) con contrapeso: salto real, pierna sujeta →
   suelta, afinar `Tst`/`fz_scale`/`analog_limit` con la telemetría `t_air_last`/`t_st_last`.
