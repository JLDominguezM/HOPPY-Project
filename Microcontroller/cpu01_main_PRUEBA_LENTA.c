...
int   MOTORS_OFF = 1;            // 1 = motores OFF (mueve a mano, solo lee encoders); 0 = motion lenta
float Kp_test[2] = {40.0, 40.0}; // P suave (N*m/rad)
float Kd_test[2] = { 2.0,  2.0}; // D suave
float u_lim_test = 2.0;          // limite SEGURO de pseudo-pwm (de 10). Sube de a poco si no mueve
float ramp_rate  = 0.15;         // rad/s -> MUY lento
float wp_hold    = 2.0;          // s en cada pose
#define NWP 9
float wp[NWP][2] = {
  { 0.000,  0.000},   // home
  { 0.349,  0.000},   // cadera +20 deg
  { 0.698,  0.000},   // cadera +40 deg
  { 0.000,  0.000},   // home
  { 0.000, -0.524},   // rodilla -30 deg
  { 0.000, -1.047},   // rodilla -60 deg
  { 0.000,  0.000},   // home
  { 0.524, -1.047},   // cuclilla suave (30,-60)
  { 0.000,  0.000},   // home
};
...
void calculate_traj(void)
{
    ...
    // PRUEBA LENTA: rampa q_ref hacia el waypoint actual a ramp_rate; al llegar, espera wp_hold y avanza.
    int j, reached = 1;
    float step = ramp_rate * T;          // T = 0.001
    for (j = 0; j < 2; j++)
    {
        float d = wp[wp_i][j] - q_ref[j];
        if      (d >  step) { q_ref[j] += step; reached = 0; }
        else if (d < -step) { q_ref[j] -= step; reached = 0; }
        else                { q_ref[j]  = wp[wp_i][j]; }
    }
    if (reached)
    {
        t_wp += T;
        if (t_wp >= wp_hold) { t_wp = 0.0; wp_i = (wp_i + 1) % NWP; }
    }
}
